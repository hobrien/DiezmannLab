...
get_genotypes <- function(data, metadata){
...
antifungals %>% 
  ...
  mutate(concentration=round(concentration, 4),
         ...
         genotype = ifelse(genotype=='WT', genotype, glue("*{genotype}*"))) %>%
  ggplot(aes(x=as.factor(concentration), 
             y=genotype, 
             fill=normalised_od)) +
    geom_tile() +
    facet_grid(gda ~ date, scales="free") +
    scale_fill_viridis_c(name="Normalised OD", option="inferno") +
...
(bf_biomass_plot <-
  bf_biomass %>% 
  get_genotypes(bf_biomass_metadata) %>% 
  make_boxplot(x=mass, y=as.factor(gda), colour=NULL, NULL, genotype, xlab="mass", ylab="GDA (µg/ml)"))

  ggplot(aes(x=mass, y=as.factor(gda))) + 
    geom_boxplot(colour='grey', fill='black', alpha=.1, outlier.alpha=0) +
...
colony_morphology %>% 
  get_genotypes(colony_morphology_metadata) %>% 
...
(heat_shock_plot <-
heat_shock %>% 
  get_genotypes(heat_shock_metadata) %>% 
...
oxidative_stress %>% 
  get_genotypes(oxidative_stress_metadata) %>%
